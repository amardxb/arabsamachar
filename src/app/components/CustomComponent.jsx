    ...
    image: ({ value }) => {
      ...
      return (
        <div className="w-full my-6 relative h-[300px] md:h-[450px]">
          <BlurImage
            src={imgUrl(value, 960)}    // was full-size; 960px is plenty for article body
...
/*     table: ({ value }) => {
      ...
      return (
        <table className="table-fixed overflow-x-scroll w-full border mb-6 mt-4">
          <thead className="bg-gray-200">
            <tr>
              {firstRow?.cells
                ? firstRow.cells.map(cell => <th key={cell} className="border border-gray-700 p-1 text-sm">{cell}</th>)
                : <tr><th colSpan="100" className="text-center">No data</th></tr>}
            </tr>
          </thead>
          <tbody>
            {remaining?.length > 0
              ? remaining.map(row => (
                  <tr key={row._key}>
                    {row?.cells
                      ? row.cells.map(cell => <td key={cell} className="border border-gray-700 p-1 text-sm">{cell}</td>)
                      : <td>No data</td>}
                  </tr>
                ))
        ...
        <thead>
          <tr className="bg-gray-200">
            {firstRow?.cells?.length > 0 ? (
              ...
              <th colSpan="100" className="text-center p-2">
                No data
              </th>
            )}
          </tr>
